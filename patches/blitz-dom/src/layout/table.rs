...
pub(crate) fn build_table_context(
    ...
    // Marketing emails sometimes put `colspan="2"` on a signature or footer
    // cell even though every row contains exactly one real cell. Browsers
    // effectively treat this as one logical column: the declared width of an
    // earlier cell remains a minimum, and the column stretches to the table's
    // full width. Modeling the redundant span as a second grid track leaves a
    // large empty strip and forces the preceding copy into the first track.
    if !cells.is_empty() && !has_multi_cell_row {
        for cell in &mut cells {
            cell.style.grid_column = taffy::Line {
                start: style_helpers::auto(),
                end: style_helpers::span(1),
            };
        }
        let min = column_sizes
            .first()
            .map(TrackSizingFunction::min_sizing_function)
            .unwrap_or_else(style_helpers::auto);
        column_sizes.clear();
        column_sizes.push(style_helpers::minmax(
            min,
            style_helpers::auto::<taffy::MaxTrackSizingFunction>(),
        ));
        max_columns = 1;
    }
    column_sizes.resize(max_columns as usize, style_helpers::auto());
    ...
    match display.inside() {
        ...
        DisplayInside::TableRow => {
            ...
            *col = 0;
            ...
            doc.nodes[node_id].children = children;
            *max_columns = (*max_columns).max(*col);
            *has_multi_cell_row |= cells.len().saturating_sub(first_cell) > 1;
        }
...
impl taffy::LayoutPartialTree for TableTreeWrapper<'_> {
    ...
    fn compute_child_layout(
        ...
        // Grid track sizing first asks each table cell for a vertical
        // intrinsic size. Inline text which wraps at the resolved column width
        // and nested layout tables can both return only a partial estimate in
        // that mode (sometimes zero, sometimes a single line). The later full
        // cell layout then paints beyond the row which was sized from that
        // estimate. Re-run vertical sizing as a full layout and retain the
        // larger result so following rows cannot overlap the cell's content.
        ...
        {
            let laid_out = self.doc.compute_child_layout(
                node_id,
            ...
            if laid_out.size.height > output.size.height {
                return laid_out;
            }
        }
        output
