...
/// Converts Swash's positive faux-italic angle to the glyph transform expected
/// by the renderer. Glyph outlines are flipped from font space (Y up) to layout
/// space (Y down) after this transform is supplied, so the skew has to be
/// negated for the top of the glyph to lean to the right.
fn synthetic_italic_transform(angle: f32) -> Affine {
        ...
        for item in line.items() {
            if let PositionedLayoutItem::GlyphRun(glyph_run) = item {
                ...
                let glyph_xform = synthesis
                    .skew()
                    .map(synthetic_italic_transform);
...
#[cfg(test)]
mod tests {
    use super::synthetic_italic_transform;
    use kurbo::{Affine, Point};

    #[test]
    fn synthetic_italic_leans_the_glyph_top_to_the_right() {
        // Glifo composes the supplied glyph transform with this Y flip when it
        // converts a font-space outline into layout coordinates.
        let transform = synthetic_italic_transform(14.0) * Affine::FLIP_Y;
        let baseline = transform * Point::new(0.0, 0.0);
        let glyph_top = transform * Point::new(0.0, 10.0);

        assert!(glyph_top.x > baseline.x);
        assert!(glyph_top.y < baseline.y);
    }
}
