...
fn derive_bitflags(input: &syn::DeriveInput, bitflags: &CssBitflagAttrs) -> TokenStream {
    ...
    for (rust_name, css_name) in bitflags.mixed_flags() {
        let rust_ident = Ident::new(&rust_name, Span::call_site());
        // Aviary patch: explicit error propagation instead of `?`.
        // The presence of `serde_fmt` in the graph (through gpui -> log/kv)
        // adds a second `impl From<_> for fmt::Error`, making the implicit
        // `From::from` from `?` ambiguous (E0282/E0283). The same applies to
        // every `match ... return Err(error)` form below.
        let serialize = quote! {
            if has_any {
                match dest.write_char(' ') {
                    Ok(()) => {}
                    Err(error) => return Err(error),
                }
            }
            has_any = true;
            match dest.write_str(#css_name) {
                Ok(()) => {}
                Err(error) => return Err(error),
            }
        };
